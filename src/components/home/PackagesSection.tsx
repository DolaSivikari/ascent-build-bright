...
import { CheckCircle2, Wrench, Paintbrush, Home, Info } from "lucide-react";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
const PackagesSection = () => {
  ...
  const [selectedPackage, setSelectedPackage] = useState<'starter' | 'refresh' | 'weekend'>('starter');

  const handlePackageCTA = (packageId: string) => {
    trackEvent('package_cta_click', { package: packageId as any });
                ...
                >
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Info className="w-4 h-4 text-muted-foreground cursor-help mb-1" />
                          </TooltipTrigger>
                          <TooltipContent side="top" className="max-w-[250px] text-xs">
                            <p>*Sample pricing — adjust to market. Upload a photo for a reliable ballpark.</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </div>
                  ...
                  <CardFooter className="flex flex-col gap-2">
                    <Button 
                      ...
                      onClick={() => handlePackageCTA(pkg.packageId)}
                    >
                      {pkg.cta}
                    </Button>
                    <Button 
                      ...
                      onClick={() => {
                        trackEvent('package_cta_click', { package: 'custom' });
                        setSelectedPackage('custom' as any);
                        setShowModal(true);
                      }}
                    >
